...
async def check_message(cog, message):
    ...
    if not message.channel.permissions_for(message.guild.me).manage_messages:
        logger.debug("Lacks permissions to delete messages in guild '%s' (%d)", message.guild.name, message.guild.id)
        return
...
async def check_member_join(cog, member):
    ...
    if not guild.me.guild_permissions.manage_roles:
        logger.debug("Lacks permissions to manage roles in guild '%s' (%d)", message.guild.name, message.guild.id)
        return
