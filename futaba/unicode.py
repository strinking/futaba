...
def _load_unicode_blocks():
    if not os.path.exists(UNICODE_BLOCKS_FILENAME):
        logger.info("Unicode blocks file '%s' does not exist, downloading...", UNICODE_BLOCKS_FILENAME)
        urlretrieve('https://unicode.org/Public/UNIDATA/Blocks.txt', filename=UNICODE_BLOCKS_FILENAME)

    blocks = []
    with open(UNICODE_BLOCKS_FILENAME) as fh:
        content = fh.read()

    for start, end, block_name in re.findall(r'([0-9A-F]+)\.\.([0-9A-F]+);\ (\S.*\S)', content):
