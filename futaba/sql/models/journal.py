...
ConfiguredChannelOutput = namedtuple(
    "ConfiguredChannelOutput", ("channel", "path", "settings")
)
logger = logging.getLogger(__name__)

__all__ = ["ConfiguredChannelOutput", "ChannelSettings", "JournalModel"]
...
class JournalModel:
    ...
    def add_journal_channel(self, channel, path, recursive):
        logger.info(
            "Adding journal channel for #%s (%d) on path %s",
            channel.name,
            channel.id,
            path,
        )
        ...
        try:
            ...
            self.journal_outputs_cache[channel][path] = ChannelSettings(
                recursive=recursive
            )
    ...
    def update_journal_channel(self, channel, path, recursive):
        logger.info(
            "Updating journal channel for #%s (%d) on path %s",
            channel.name,
            channel.id,
            path,
        )
    ...
    def delete_journal_channel(self, channel, path):
        logger.info(
            "Deleting journal channel for #%s (%d) on path '%s'",
            channel.name,
            channel.id,
            path,
        )
    ...
    def has_journal_channel(self, channel, path):
        logger.info(
            "Checking for journal channel on #%s (%d) for path '%s'",
            channel.name,
            channel.id,
            path,
        )
